window.ProjectsController = function($scope, $http) {
  	var _this = this;
  	...
  	$scope.editTodoList = function($event) {
    	$($event.srcElement).closest('.block_top').find("input.disabled").removeClass('disabled').addClass('enabled').removeAttr('disabled');
  		$event.preventDefault(true);
  	}
